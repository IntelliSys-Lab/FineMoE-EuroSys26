...
def draw_heatmap(ax, mat, vmin=None, vmax=None):
    # Rows are layers; columns are experts.
...
# Figure 3a: coarse and fine heatmaps.
heat_coarse = {}
heat_fine = {}
...
for (dataset, model), coarse_mat in heat_coarse.items():
    ...
    mid_gap_idx = next((i for i in range(len(chosen_iters) - 1)
                        if chosen_iters[i + 1] > chosen_iters[i] + 1), None)
    ...
    ax_right.annotate("aggregate",
                      ...
                      xytext=(-0.075, 0.5), textcoords=("axes fraction", "axes fraction"),
...
# Figure 3b: coarse and fine entropy.
...
# Figure 3c: entropy across steps.
if len(lines) == 0:
