...
void ArcherTaskPool::SetOffloadMethod(const std::string& offload_method)
{
    offload_method_ = offload_method;
}
...
bool ArcherTaskPool::RemoveCachedSparseNode(const NodePtr& node, int device_id)
{
    ...
    if (cache_size > cache_limit) {
        ...
        if (offload_method_ == "Mixtral-Offloading") {
            for (auto& n : device_nodes) { node_access_time.push_back(n->last_access_time); }
        } 
        else if (offload_method_ == "ProMoE") {
            for (auto& n : device_nodes) { node_access_time.push_back(n->last_access_time); }
        }
        else if (offload_method_ == "DeepSpeed") {
            for (auto& n : device_nodes) { node_access_time.push_back(n->last_access_time); }
        }
        else if (offload_method_ == "MoE-Infinity") {
            for (auto& n : device_nodes) { node_access_time.push_back(n->incache_visit_count); }
        } 
        else if (offload_method_ == "FineMoE") {
            for (auto& n : device_nodes) {
