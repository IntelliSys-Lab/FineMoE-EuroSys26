...
This repo contains a demo implementation of the paper, [Taming Latency-Memory Trade-Off in MoE-Based LLM Serving via Fine-Grained Expert Offloading](https://arxiv.org/pdf/2502.05370v1).
...
## EuroSys'26 AE Fast Testing

To facilitate EuroSys'26 AE process, we provide a personal GPU machine pre-configured for running this demo. To access the machine, please follow these steps:

1. Go to [Download - ZeroTier](https://www.zerotier.com/download/) and download the client.

2. Launch ZeroTier client and request to join our network using ID: **4753cf475f57acae**
![ZeroTier snapshot](zerotier-snapshot.png)

3. Send a notice in the HotCRP comment to confirm your address with us and share your public key with us.

4. We will approve your ZeroTier join request and add your public key to the machine as soon as possible.

5. After joining our network, you should be able to ssh into the machine via:
```
ssh root@10.144.178.34
```

6. The demo environment is under `/root/FineMoE-EuroSys26/demo`. You can directly jump to [Step 6](#step-6) for testing baselines and to [Results and Figures](#results-and-figures) for visualizing the results.

We kindly ask AE reviewers to coordinate usage of the provided machine to avoid overlapping operations. Concurrent runs can interfere with each other’s testing.
...
6. Execute the baselines. We provide the following baselines in this demo:
```
python eval.py --offload mixoff  # Mixtral-Offloading
python eval.py --offload promoe  # ProMoE
python eval.py --offload moeinf  # MoE-Infinity
python eval.py --offload finemoe  # FineMoE
```

## Results and Figures

After baselines are executed, you should be able to check the results as CSV files under [`demo/results`](https://github.com/IntelliSys-Lab/FineMoE-EuroSys26/tree/master/demo/results).

We provide scripts to plot the results using Matplotlib:
```
python plot_entropy.py
python plot_latency.py
```
You should be able to find the figures under [`demo/figures`](https://github.com/IntelliSys-Lab/FineMoE-EuroSys26/tree/master/demo/figures) after the script completes.
