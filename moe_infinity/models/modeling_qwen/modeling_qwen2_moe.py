...
class Qwen2MoeMLP(nn.Module):
    ...
    def forward(self, x):
        x_gate_proj = self.act_fn(self.gate_proj(x))
        x_up_proj = x_gate_proj * self.up_proj(x)
        x_down_proj = self.down_proj(x_up_proj)
        return x_down_proj
...
class SyncQwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.offload_method == "MoE-Infinity":
            ...
            layer_end = min(layer_start + 1, self.expert_tracer.num_layers)
            if layer_start < self.expert_tracer.num_layers:
                for seq_id in self.seq_id_list:
                    ...
                    for i in range(sequence_length):
                        ...
                        expert_matrix_s = expert_matrix.clone()
                        self.expert_tracer.update_preds(
                            seq_id, iter_id, expert_matrix_s, layer_start, layer_end)
                        layer_end_s = layer_end + self.prefetch_distance
                        layer_end_s += int(self.expert_tracer.num_layers /
                                           2) if sequence_length == 1 else 0
                        expert_matrix_s = expert_matrix_s[:layer_end_s, :] + 1e-6
                        self.expert_prefetcher.prefetch_experts(
                            expert_matrix_s, expert_prob_map)
            ...
            if layer_start < self.expert_tracer.num_layers:
                ...
                for seq_id, per_batch_indices in zip(self.seq_id_list, spec_indices):
                    for i, idxs_row in enumerate(per_batch_indices):
                        ...
                        expert_matrix[:layer_end+8, idxs_row] = 1.0
                        iter_id = i if sequence_length > 1 else -1
                        self.expert_tracer.update_preds(
                            seq_id, iter_id, expert_matrix, layer_start, layer_end)
                        expert_matrix += 1e-6 if sequence_length > 1 else 0
                        self.expert_prefetcher.prefetch_experts(
                            expert_matrix, expert_prob_map)

        elif self.offload_method == "ProMoE":
            layer_start = self.layer_id + 2
            layer_end = min(layer_start + 1, self.expert_tracer.num_layers)
            if layer_start < self.expert_tracer.num_layers:
                spec_gate = self.moe_layers[layer_start].gate
                spec_hidden_states = hidden_states.clone()
                ...
                for seq_id, per_batch_indices in zip(self.seq_id_list, spec_indices):
                    for i, idxs_row in enumerate(per_batch_indices):
                        ...
                        expert_prob_map = torch.zeros_like(expert_matrix)
                        expert_matrix[:layer_end+4, idxs_row] = 1.0
                        iter_id = i if sequence_length > 1 else -1
                        self.expert_tracer.update_preds(
                            seq_id, iter_id, expert_matrix, layer_start, layer_end)
                        self.expert_prefetcher.prefetch_experts(
...
class Qwen2MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2MoeConfig, layer_idx: int):
        ...
        if (layer_idx not in config.mlp_only_layers) and (
            ...
            self.mlp = SyncQwen2MoeSparseMoeBlock(config)
        else:
...
class Qwen2MoeModel(Qwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        engine_warmup = time.time() if not past_seen_tokens else None
        ...
        if self.offload_method == "FineMoE":
            ...
            for seq_id, embeds in zip(self.seq_id_list, inputs_embeds):
                ...
                self.timers[seq_id][0] += time.time() - \
                    engine_warmup if not past_seen_tokens else 0
        elif self.offload_method == "MoE-Infinity":
            for seq_id in self.seq_id_list:
                expert_matrix, expert_prob_map = self.expert_predictor.predict(
                    seq_id, 0)
        ...
        for i, decoder_layer in enumerate(self.layers[:8]):
            ...
            if i == 7 or not past_seen_tokens:
                break
