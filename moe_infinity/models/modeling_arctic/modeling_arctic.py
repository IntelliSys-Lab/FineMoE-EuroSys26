...
class ArcticAttention(nn.Module):
    ...
    def __init__(self, config: ArcticConfig, layer_idx: Optional[int] = None,  **kwargs):
        ...
        self.use_deepspeed_implementation = USE_DEEPSPEED_MOE_ARG in kwargs and kwargs[
            USE_DEEPSPEED_MOE_ARG]
...
def get_arctic_linear(input_dim,
    ...
    if is_deepspeed_available():
        if ds_optimized_lora_config is not None:
            ...
            ds_optimized_lora_config.base_weight_sharding = torch.distributed.get_world_size(
            ) if ds_optimized_base_weight_sharding else 1
...
class ArcticMoE(nn.Module):
    ...
    def _moe_foreward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        bincount = torch.bincount(
            expert_idx, minlength=self.num_experts).tolist()
...
class ArcticDecoderLayer(nn.Module):
    def __init__(self, config: ArcticConfig, layer_idx: int, **kwargs):
        ...
        self.block_sparse_moe = SyncArcticMoeBlock(
            config, layer_id=layer_idx, **kwargs)
        ...
        **kwargs,
        ...
        if self.parallel_attn_mlp_res:
            ...
            hidden_states = self.post_attention_layernorm(
                residual_input)  # parallel attn mlp has the same input
            hidden_states, gate_loss = self.block_sparse_moe(hidden_states)
            hidden_states = residual_residual + \
                hidden_states.view(
                    residual_residual.shape[0], residual_residual.shape[1], -1)
        else:
...
class ArcticModel(ArcticPreTrainedModel):
    ...
    def forward(
        ...
        next_cache = None
        if use_cache:
            next_cache = next_decoder_cache.to_legacy_cache() if use_legacy_cache and hasattr(
                next_decoder_cache, 'to_legacy_cache') else next_decoder_cache
...
class ArcticForCausalLM(ArcticPreTrainedModel):
    ...
    def state_dict(self, *args, **kwargs):
        ...
        for param_name in expert_params:
            param_tensor = state_dict[param_name].to('cuda:0')
            output = [torch.zeros_like(param_tensor)
                      for _ in range(world_size)]
            torch.distributed.gather(
                param_tensor, gather_list=output if rank == 0 else None, dst=0, group=None)
            ...
            for gather_rank, gather_param in enumerate(output):
                ...
                new_expert_number = gather_rank * experts_per_rank + \
                    self._expert_number_from_param_name(param_name)
                new_param_name = re.sub(
                    r'(experts\.)(\d+)(\.)', rf'\g<1>{new_expert_number}\3', param_name)
        ...
        if self.is_deepspeed_lora:
            ...
            for param_name in list(state_dict.keys()):
                if param_name.endswith("base_weight"):
                    base_weight = state_dict[param_name].to('cuda:0')
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        local_expert_range = range(
            num_local_experts * rank, num_local_experts * rank + num_local_experts)
        ...
        for param_name in expert_params_to_keep:
            ...
            new_expert_number = self._expert_number_from_param_name(
                param_name) % num_local_experts
            new_param_name = re.sub(
                r'(experts\.)(\d+)(\.)', rf'\g<1>{new_expert_number}\3', param_name)
            ...
            ds_moe_path = "mlp.deepspeed_moe.experts.deepspeed_experts".split(
                '.')
            new_param_name = split_param_name[0:idx] + \
                ds_moe_path + split_param_name[idx+1:]
        ...
        for param_name in gate_params:
            ...
            print(
                f'Gating: {rank=}, renaming {param_name} -> {new_param_name}')
            state_dict[new_param_name] = state_dict.pop(param_name)
        ...
        if self.is_deepspeed_lora:
            ...
            for param_name in local_state_dict:
                ...
                if 'deepspeed_moe' in incoming_param_name:
                    ...
                    assert shape_incoming[1] == shape_local[1] * \
                        world_size, "weights should be sharded equally across world size"
                    incoming_param = incoming_param[:, rank *
                                                    shape_local[1]: (rank+1)*shape_local[1]]
                print(
