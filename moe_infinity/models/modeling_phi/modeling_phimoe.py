...
class Phi3LongRoPEScaledRotaryEmbedding(nn.Module):
    ...
    def __init__(self, dim, config):
        ...
        self.original_max_position_embeddings = config.rope_scaling[
...
class PhiMoEAttention(nn.Module):
    ...
    def __init__(self, config: PhiMoEConfig, layer_idx: Optional[int] = None):
        ...
        self.k_proj = nn.Linear(self.hidden_size, self.num_key_value_heads *
                                self.head_dim, bias=self.config.attention_bias)
...
class PhiMoEBlockSparseTop2MLP(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states_w1 = self.act_fn(self.w1(hidden_states))
        hidden_states_w3 = hidden_states_w1 * self.w3(hidden_states)
        hidden_states_w2 = self.w2(hidden_states_w3)
        return hidden_states_w2
...
def sparsemixer(scores, top_k, jitter_eps, training):
    ...
    if training:
        selected_experts = (
            masked_gates - torch.empty_like(
                masked_gates, memory_format=torch.legacy_contiguous_format).exponential_().log()
    ...
    if training:
        ...
        mask_for_one = torch.add(0.3333, mask_for_one,
                                 alpha=0.6667).type_as(masked_gates)
    ...
    with torch.no_grad():
        # compute mask for sparsity
        mask_logits_threshold, max_ind = masked_scores.max(
            dim=-1, keepdim=True)
        factor = scores.abs().clamp(min=mask_logits_threshold)
        mask_logits_threshold = (
            (mask_logits_threshold - scores) / factor
    ...
    # apply mask
    masked_gates_top2 = masked_scores.masked_fill(
        mask_logits_threshold, float('-inf'))
    if training:
        selected_experts_top2 = (
            masked_gates_top2 - torch.empty_like(
                masked_gates_top2, memory_format=torch.legacy_contiguous_format).exponential_().log()
        ).max(dim=-1)[1].unsqueeze(-1)  # gumbel sampling, more robust than than the multinomial method
    else:
    ...
    if training:
        ...
        # 1 -> 1.0 & 0 -> 1./3: lambda x: (x + 0.5) / 1.5
        mask_for_one_top2 = torch.add(
            0.3333, mask_for_one_top2, alpha=0.6667).type_as(masked_gates_top2)
...
class SyncPhiMoESparseMoeBlock(nn.Module):
    """
    This implementation is
    strictly equivalent to standard MoE with full capacity (no
    dropped tokens). It's faster since it formulates MoE operations
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.offload_method == "MoE-Infinity":
            layer_start = self.layer_id + 1
            layer_end = layer_start + 1
            if layer_start < self.expert_tracer.num_layers:
                for seq_id in self.seq_id_list:
                    expert_matrix, expert_prob_map = self.expert_predictor.predict(
                        seq_id, layer_start)
                    for i in range(sequence_length):
                        self.expert_prefetcher.prefetch_experts(
                            expert_matrix, expert_prob_map)
                        iter_id = i if sequence_length > 1 else -1
                        self.expert_tracer.update_preds(
                            seq_id, iter_id, expert_matrix, layer_start, layer_end)

        elif self.offload_method == "Mixtral-Offloading":
            layer_start = self.layer_id + 1
            layer_end = layer_start + 1
            if layer_start < self.expert_tracer.num_layers:
                spec_gate = self.moe_layers[layer_start].gate
                spec_hidden_states = hidden_states.clone()
                try:
                    spec_expert_probs = F.softmax(
                        spec_gate(spec_hidden_states), dim=1, dtype=torch.float)
                except:
                    spec_hidden_states = spec_hidden_states.to(
                        spec_gate.weight.device)
                    spec_expert_probs = F.softmax(
                        spec_gate(spec_hidden_states), dim=1, dtype=torch.float)
                finally:
                    _, spec_indices = torch.topk(
                        spec_expert_probs, self.top_k, dim=-1)
                    spec_indices = spec_indices.view(
                        -1, sequence_length, self.top_k).to(self.device)

                for seq_id, per_batch_indices in zip(self.seq_id_list, spec_indices):
                    for i, idxs_row in enumerate(per_batch_indices):
                        expert_matrix = torch.zeros(
                            (self.expert_tracer.num_layers, self.num_experts),
                            dtype=torch.float32,
                            device=self.device,
                        )
                        expert_prob_map = torch.zeros_like(expert_matrix)
                        expert_matrix[layer_start].index_fill_(
                            0, idxs_row, 1.0)
                        self.expert_prefetcher.prefetch_experts(
                            expert_matrix, expert_prob_map)
        ...
        elif self.offload_method == "ProMoE":
            layer_start = self.layer_id + self.prefetch_distance - 1
            layer_end = layer_start + 1
            if layer_start < self.expert_tracer.num_layers:
                spec_gate = self.moe_layers[layer_start].gate
                spec_hidden_states = hidden_states.clone()
        ...
        elif self.offload_method == "FineMoE":
            input_trajs = []
            for trace_entry in self.expert_tracer.trace.values():
                if sequence_length > 1:
                    chosen_iters = trace_entry.iters
                else:
            ...
            assert not (input_trajs == 0).all(
                dim=-1).any(), "Layer with all-zero expert probs exist"
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_hidden_states = expert_layer(current_state).to(
                routing_weights.device) * routing_weights[top_x_list, idx_list, None]
...
class PhiMoEModel(PhiMoEPreTrainedModel):
    ...
    def forward(
        ...
        if self.offload_method == "FineMoE":
            assert self.expert_map_matcher is not None
            for seq_id, embeds in zip(self.seq_id_list, inputs_embeds):
                self.expert_map_matcher.embed_prefetch(seq_id, embeds)
        elif self.offload_method in "MoE-Infinity":
            for seq_id in self.seq_id_list:
                expert_matrix, expert_prob_map = self.expert_predictor.predict(
                    seq_id, 0)
                for i in range(seq_len):
                    ...
                    self.expert_tracer.update_preds(
                        seq_id, iter_id, expert_matrix, 0, 1)
        elif self.offload_method in "DeepSpeed":
            expert_matrix = torch.ones(
                (self.expert_tracer.num_layers,
                    self.expert_tracer.num_experts),
                dtype=torch.float32,
                device=self.device,
            )
            for seq_id in self.seq_id_list:
...
class PhiMoEForCausalLM(PhiMoEPreTrainedModel):
    ...
    def forward(
        ...
        for seq_id in self.seq_id_list:
            self.timers[seq_id].append(time.time())
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values and self.config.rope_scaling and input_ids.shape[1] >= self.config.original_max_position_embeddings + 1:
            past_length = past_key_values.seen_tokens if isinstance(
                past_key_values, Cache) else past_key_values[0][0].shape[2]
