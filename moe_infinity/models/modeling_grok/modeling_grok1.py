    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = self.max_attn_val * \
            F.tanh(attn_weights / self.max_attn_val)
...
class SyncGrokMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        expert_probs = F.softmax(router_logits, dim=1, dtype=torch.float)
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        routing_weights, selected_experts = torch.topk(
            routing_weights, self.top_k, dim=-1
        )
        ...
        if self.offload_method == "MoE-Infinity":
            for i in range(batch_size):
                ...
                expert_matrix = self.expert_predictor.predict(
                    ...
                    expert_index[i],
                    self.layer_id,
                    expert_probs[i*sequence_length:(i+1)*sequence_length],
                )
...
class DecoderLayer(nn.Module):
    def __init__(
        ...
        self.moe_block = SyncGrokMoeBlock(
...
class Grok1Model(Grok1PretrainedModel):
    ...
    def forward(
        ...
        # Record semantic hints
        inputs_embeds_to_record = inputs_embeds.detach().cpu().float()
        for i, seq_id in enumerate(self.seq_id_list):
            embed = inputs_embeds_to_record[i].mean(dim=0).numpy()
            self.expert_tracer.update_embed(seq_id, embed)
