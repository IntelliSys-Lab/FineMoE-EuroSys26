...
class ExpertMapMatcher():
    ...
    def _select_by_cumsum(self, probs: torch.Tensor, threshold: torch.Tensor, top_k: int):
        ...
        k_before = (csum < th).sum(-1)
        k_raw = k_before + (th.squeeze(-1) > 0).long()
        k_nonzero = torch.clamp(k_raw, min=top_k, max=vals.size(-1))
        k = torch.where(has_pos, k_nonzero, torch.zeros_like(k_raw))
    ...
    def embed_prefetch(self, seq_id: int, input_embeds: torch.Tensor):
        ...
        if scores is not None and maps is not None:
            layer_start = 0
            layer_end = min(
                self.num_layers, self.prefetch_distance) if seq_len > 1 else layer_start + 1
            ...
            for i, (s, m) in enumerate(zip(scores, maps)):
                ...
                pred_map[layer_start:layer_end] += 1e-6
    ...
    def traj_prefetch(self, seq_id: int, input_trajs: torch.Tensor):
        ...
        layer_start = num_layers_obs + \
            self.prefetch_distance if seq_len > 1 else num_layers_obs
        if layer_start < self.num_layers:
            layer_end = min(layer_start + self.prefetch_distance, 8)
...
class OffloadEngine(object):
    ...
    def __enter__(self):
        ...
        def from_pretrained_decorator(orig_from_pretrained: Callable) -> Callable:
            ...
            def archer_from_pretrained(cls, *args, **kwargs):
                ...
                model.model.timers = self.timers
    ...
    # clean runtime hooks
    def clean_up(self):
        moe_infinity.models.modeling_mixtral.SyncQwen2MoeSparseMoeBlock = (
