...
from moe_infinity.memory import ExpertTracer, ExpertPredictor
...
class ExpertMapStore():
    ...
    def add(self, embeds, expert_maps):
        ...
        if rem > 0:
            ...
            S_e = self._cosine_sim(
                embeds[-rem:], self.store_embed[:self.data_size])
            A_t = expert_maps[-rem:].reshape(rem, -1)
            B_t = self.store_traj[:self.data_size].reshape(self.data_size, -1)
            S_t = self._cosine_sim(A_t, B_t)
    ...
    def match_embed(self, embeds):
        ...
        embeds = self._ensure_tensor(embeds, shape_last=(self.embed_dim,))
        S = self._cosine_sim(
            embeds, self.store_embed[:self.data_size])
        scores, argmax = S.max(dim=1)
    ...
    def match_traj(self, trajs):
        ...
        L_obs = trajs.shape[1]
        S = self._cosine_sim(
            trajs.reshape(trajs.size(0), -1),
            self.store_traj[:self.data_size, :L_obs,
                            :].reshape(self.data_size, -1)
        )
        scores, argmax = S.max(dim=1)
        maps = self.store_traj[:self.data_size][argmax]
        return scores, maps
...
class ExpertMapMatcher():
    ...
    def _select_by_cumsum(self, probs: torch.Tensor, threshold: torch.Tensor, top_k: int) -> torch.Tensor:
        ...
        base = csum >= th
        ensure_k = (torch.arange(vals.size(-1), device=vals.device) < top_k)
        ensure_k = ensure_k.unsqueeze(0).expand_as(base)
        keep_sorted = base | ensure_k
    ...
    def _layer_decay_weights(self, layer_start: int, layer_end: int) -> torch.Tensor:
        ...
        if layer_end > layer_start:
            rng = torch.arange(L, device=self.device)
    ...
    def process_expert_map(self, layer_start: int, layer_end: int,
        ...
        select_threshold = torch.clamp(1.0 - score, 0.0, 1.0)
        prefetch_priority_map = self._select_by_cumsum(
            probs, select_threshold, self.top_k)
    ...
    def embed_prefetch(self, seq_id: int, input_embeds: torch.Tensor):
        ...
        if scores is not None and maps is not None:
            layer_start = 0
            if seq_len > 1:
                layer_end = min(self.num_layers, self.prefetch_distance)
            else:
            ...
            for i, (s, m) in enumerate(zip(scores, maps)):
                ...
                self.expert_prefetcher.prefetch_experts(pred_map, prob_map)
    ...
    def traj_prefetch(self, seq_id: int, input_trajs: torch.Tensor):
        ...
        if seq_len > 1:
            layer_start = num_layers_obs + self.prefetch_distance - 1
        else:
            layer_start = num_layers_obs
        if layer_start < self.num_layers:
            ...
            if scores is not None and maps is not None:
                for i, (s, m) in enumerate(zip(scores, maps)):
                    ...
                    iter_id = i if seq_len > 1 else -1
                    self.expert_tracer.update_preds(
                        seq_id, iter_id, pred_map, layer_start, layer_start + 1)
...
class OffloadEngine(object):
    ...
    def __init__(
        ...
        self.expert_predictor = ExpertPredictor(config, prefetch_distance)

        self.expert_map_store = ExpertMapStore(
            capacity=capacity,
            num_layers=self.expert_predictor.num_layers,
            num_experts=self.expert_predictor.num_experts,
            embed_dim=self.expert_predictor.embed_dim,
        ...
        self.offload_method = offload_method
        self.prefetch_distance = prefetch_distance
        self.device = torch.device(device)
    ...
    def __enter__(self):
        ...
        moe_infinity.models.modeling_mixtral._old_sparse_mlp = (
            moe_infinity.models.modeling_mixtral.MixtralSparseMoeBlock
        )
        moe_infinity.models.modeling_mixtral.MixtralSparseMoeBlock = (
            SyncMixtralSparseMoeBlock
        )
        moe_infinity.models.modeling_qwen.modeling_qwen2_moe._old_sparse_mlp = (
            moe_infinity.models.modeling_qwen.modeling_qwen2_moe.Qwen2MoeSparseMoeBlock
        )
        moe_infinity.models.modeling_qwen.modeling_qwen2_moe.Qwen2MoeSparseMoeBlock = (
            SyncQwen2MoeSparseMoeBlock
        )
        moe_infinity.models.modeling_phi.modeling_phimoe._old_sparse_mlp = (
            moe_infinity.models.modeling_phi.modeling_phimoe.PhiMoESparseMoeBlock
        )
        moe_infinity.models.modeling_phi.modeling_phimoe.PhiMoESparseMoeBlock = (
            SyncPhiMoESparseMoeBlock
        )
        ...
        def from_pretrained_decorator(orig_from_pretrained: Callable) -> Callable:
            ...
            def archer_from_pretrained(cls, *args, **kwargs):
                ...
                model.model.expert_predictor = self.expert_predictor
                model.model.expert_map_matcher = self.expert_map_matcher
                model.model.offload_method = self.offload_method
                model.model._device = self.device
                model.timers = self.timers

                module_idx = 0
                self.expert_layer_modules = []
                for module in model.modules():
                    if (
                        isinstance(module, SyncMixtralSparseMoeBlock)
                        or isinstance(module, SyncQwen2MoeSparseMoeBlock)
                        or isinstance(module, SyncPhiMoESparseMoeBlock)
                    ):
                        ...
                        module.offload_method = self.offload_method
                    ...
                    if (
                        isinstance(module, MixtralBlockSparseTop2MLP)
                        or isinstance(module, Qwen2MoeMLP)
                        or isinstance(module, PhiMoEBlockSparseTop2MLP)
                    ):
            ...
            if "expert" in key:
                for expert_idx, expert_tensors in enumerate(tensors):
                    expert_key = (
                        ...
                        if self.config.model_type not in ["mixtral", "qwen2_moe", "phimoe", "grok-1", "arctic"]
