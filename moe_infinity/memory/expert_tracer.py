...
class ExpertTracer:
    ...
    def update_entry(self, seq_id, expert_list: torch.Tensor, layer_idx: int,
        ...
        # normalize inputs to device/dtypes
        ...
        # Update the global per-layer counts once (fast path)
        row_global = trace_entry.matrix[layer_idx]  # [E] on device
        if num_tokens > 1:
            # vectorized across tokens
            self._row_add_bincount(row_global, expert_list)
        else:
            self._row_add_index_add(row_global, expert_list[0])

        if num_tokens > 1:
            # Prefill: update each token's per-iter tensors
            ...
            for token_idx in range(num_tokens):
                ...
                idxs = expert_list[token_idx]  # [top_k] on device
                # nodes row update
    ...
    def find_most_similar(self, matrix: torch.Tensor, layer_idx: int) -> torch.Tensor:
        ...
        valid_mask = trace_sum > 0
        if not valid_mask.any():
            return self.trace_collection.sum(dim=0)

        trace_full = self.trace_collection[valid_mask]
        trace_prefix = trace_full[:, :layer_idx, :]

        A = matrix[:layer_idx, :].reshape(
            1, -1).to(self.device, dtype=self.dtype)
        B = trace_prefix.reshape(trace_prefix.size(0), -1)
        S = self.expert_map_store._cosine_sim(A, B)

        idx = int(torch.argmax(S, dim=1).item())
        matched_matrix = trace_full[idx]
