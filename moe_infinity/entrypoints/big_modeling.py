...
class MoE:
    ...
    def __init__(
        ...
        self.offload_method = config.pop('offload_method')
        ...
        self.engine = OffloadEngine(
            ...
            config=model_config,
            offload_method=self.offload_method,
    ...
    def _configure_hook(self, input_ids: torch.LongTensor):
        if self.arch == "mixtral":
            moe_infinity.models.modeling_mixtral.apply_rotary_pos_emb = (
                apply_rotary_pos_emb
            )
        elif self.arch == "qwen":
            moe_infinity.models.modeling_qwen.modeling_qwen2_moe.apply_rotary_pos_emb = (
                apply_rotary_pos_emb
            )
        elif self.arch == "phi":
            moe_infinity.models.modeling_phi.modeling_phimoe.apply_rotary_pos_emb = (
                apply_rotary_pos_emb
            )
        elif self.arch == "grok":
        ...
        for seq_id in self.seq_id_list:
            self.engine.timers[seq_id] = [time.time()]
